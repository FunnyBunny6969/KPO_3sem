...
namespace MFST
{
	...
	bool Mfst::start()
	{
		...
		case LENTA_END:
			MFST_TRACE4("-------> NS_LENTA_END")
				*log->stream << "--------------------------------------------------------------" << endl;
			*log->stream << setw(4) << left << "Âñåãî ñòðîê " << lenta_size << ", Ñèíòàêñè÷åñêèé àíàëèç âûïîëíåí áåç îøèáîê" << endl;
			rc = true;
			break;
		case SURPRISE:
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: ñòðîêà %d, %s", err.id, lex.lexTable.table[lpos].sn, err.message);
			rc = buf;
	...
	{
		*log->stream << "== ÄÅÐÅÂÎ ÐÀÇÁÎÐÀ ==" << endl;
